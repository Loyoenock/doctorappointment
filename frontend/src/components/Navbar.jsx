...
const Navbar = () => {
  ...
  // Show profile and menu items only if user is logged in
  const [showMenu, setShowMenu] = useState(false)
  const [token, setToken] = useState(true)

  return (
    <nav className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
      <img className='width-44 cursor-pointer' src={assets.logo} onClick={() => {navigate('/')}}  />
      ...
      <div className='flex items-center gap-4'>
        ...
        <img src={assets.menu_icon} alt='' onClick={() => setShowMenu(true)} className='w-6 md:hidden' />    
        {/* ------------------- Mobile Menu----------------------- */}
